...
SARIFOUT="$4"
SLITHERVER="$5"
SLITHERARGS="--json slither-result.json"
...
if [[ -z "$SLITHERARGS" ]]; then
    echo 'TARGET='"$TARGET"
    echo 'SARIFFLAG='"SARIFFLAG"
    echo 'IGNORECOMPILEFLAG='"$IGNORECOMPILEFLAG"
    echo 'FAILONFLAG='"$FAILONFLAG"
    ...
    echo "[-] Args: SLITHERARGS=$SLITHERARGS TARGET=$TARGET SARIFFLAG=$SARIFFLAG IGNORECOMPILEFLAG=$IGNORECOMPILEFLAG FAILONFLAG=$FAILONFLAG CONFIGFLAG=$CONFIGFLAG STDOUTFILE=$STDOUTFILE"
