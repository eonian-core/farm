...
function parseUnsupportedOperationError(
  { message }: ethers.UnsupportedOperationError
...
function parseCallExceptionError(
...
function parseActionRejectionError(
  error: ethers.ActionRejectedError
): [string, TransactionErrorSeverity] {
  switch (error.reason) {
    case "rejected": {
      return [
    ...
    case "pending": {
      return [
        "Transaction Rejected: Another transaction is pending.",
        TransactionErrorSeverity.WARNING,
      ];
    }
    case "expired": {
      return [
        "Transaction Rejected: The transaction has been expired.",
        TransactionErrorSeverity.ERROR,
      ];
    }
  }
